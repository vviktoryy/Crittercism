...
public class HttpConnection  extends AsyncTask {
    ...
    private Boolean DoHttpURLConnection(){
    ...
    private Boolean DoApacheConnection(){
        long timeBeg,timeEnd;
        ...
        try {
            ...
            timeEnd =System.currentTimeMillis();
            time = timeEnd-timeBeg;
            if(method.equals("get") && status < HttpStatus.SC_BAD_REQUEST)  {
    ...
    private Boolean DoOkHttpConnection(){
        long timeBeg,timeEnd;
    ...
    private String CreateNetworkAction(){
        ...
        if (connType==0){//HttpURLConnection
            conn = DoHttpURLConnection();
        }else if (connType==1) {//org.apache.http.client.HttpClient
            conn = DoApacheConnection();
        } else {//OkHttpClient
            conn = DoOkHttpConnection();
        }
        if(conn) return "(" + status + ") " + url;
        else return null;
    }
}
